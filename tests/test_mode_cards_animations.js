...
// Validates 3D tilt tracking, holographic foils, theme accents, rotating neon
// border runners, and theme kinetic micro-animations on all 4 mode cards.
// ==============================================================================
...
assert(heroCode.includes('mode-card-foil'), 'hero.js must include holographic foil overlay on mode cards');
...
console.log('  ✅ PASS: hero.js markup contains 3D classes, theme identifiers, foils, and micro-animations');

// 2. Verify hero.js dynamic 3D tilt and mouse tracking logic
...
assert(heroCode.includes("setProperty('--card-foil-x'"), 'hero.js must update dynamic cursor foil X');
assert(heroCode.includes("setProperty('--card-foil-y'"), 'hero.js must update dynamic cursor foil Y');
assert(heroCode.includes("audio.playTalismanChime"), 'hero.js must trigger subtle musical chime on card hover');
console.log('  ✅ PASS: hero.js implements smooth 3D perspective tilt, laser foil cursor tracking, and audio chimes');
...
assert(cssCode.includes('.mode-card-foil'), 'css must define .mode-card-foil');
assert(cssCode.includes('rotateModeBorder'), 'css must define rotating neon border runner keyframes');
...
console.log('  ✅ PASS: css/components.css contains full mode card 3D tilt, theme glows, foil beam, and animations');
